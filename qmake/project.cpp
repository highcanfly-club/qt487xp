...
QMakeProject::read(uchar cmd)
{
    if(cfile.isEmpty()) {
        ...
        if(cmd & ReadConf) {             // parse mkspec
            ...
            debug_msg(0, "Looking for mkspec %s in (%s)", qmakespec.toLatin1().constData(),
            ...
            if(qmakespec.isEmpty()) {
                ...
                if (qmakespec.isEmpty()) {
                    QString mkspec = QDir::currentPath() + QDir::separator() + ".." + QDir::separator() + "default";
                    debug_msg(0, "Looking finally for mkspec %s", mkspec);
                    QFileInfo default_info(mkspec);
                    if (default_info.exists() && default_info.isDir()) {
                        qmakespec = mkspec;
                    }
